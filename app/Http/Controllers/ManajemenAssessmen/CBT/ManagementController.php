...
class ManagementController extends Controller
{
    ...
    {
        $modul_program = MgtProgram::where('program_id', $program_id)->where('status', 1)->get();
        foreach ($modul_program as $key) {
            foreach($key->submodul as $kuk){
                if(!empty($kuk->id))
                {
    
                    $submodul_id[] = $key->submodul_id;
                    $modul_id[]    = $key->modul_id;
                }
            }
           
        }
        $modul          = CompetenceUnit::orderBy('name', 'ASC')->where('status', 1)->get(); 
        $submodul       = CompetenceKUK::orderBy('name', 'ASC')->where('status', 1)->pluck('name', 'id');
        
    //    return $submodul_id; exit;
        return view('ManajemenAssessmen.cbt.ManagementController.edit',
                    compact('submodul_id','modul_id', 'modul_program', 'modul', 'submodul')
                );
    }
    ...
    public function AjaxMgtProgramGetData()
    {
        ...
        ->addColumn('nama_program', function ($program_dtl) {
            ...
            $btn_action .= '<a data-toggle="tooltip" title="Edit Program '.$MgtProgram->name.'" href="'.route('ujian-komputer.management.edit', ['program_id' => $MgtProgram->program_id]) .'"  class="la la-edit modalIframe"></a>&nbsp;';
    ...
    public function update(Request $request)
    {
        ...
        $nama_program = Program::where('id', $request->program)->first(); //Nama Program
        DB::beginTransaction();
        $success_trans = false;

        try {
            MgtProgram::where('program_id', $request->program)
                    ->where('status', 1)
                    ->update(['status'     => 0,
                        'deleted_at'    => date('Y-m-d H:i:s'),
                        'user_deleted'    => \Auth::id()
                    ]);
            // new insert to tb:program_dtl
            foreach ($arrtree as $submodul_id) {
                $element_id                  =  DB::table('competence_kuk')->where('id', $submodul_id)->value('competence_element_id');
                $unit_id                  =  DB::table('competence_elements')->where('id', $element_id)->value('competence_unit_id');
                $unit                 =  DB::table('competence_units')->where('id', $unit_id)->value('id');

               
                $program_dtl               = new MgtProgram;
                $program_dtl->name         = $nama_program->name;
                $program_dtl->modul_id     = $unit;
                $program_dtl->submodul_id  = $submodul_id;
                $program_dtl->program_id   = $request->program;
                $program_dtl->status        = 1;
                $program_dtl->created_at = date('Y-m-d H:i:s');
                $program_dtl->user_created = \Auth::id();
                $program_dtl->save();
                // $program_dtl->mgt_kuk()->sync($submodul_id);
