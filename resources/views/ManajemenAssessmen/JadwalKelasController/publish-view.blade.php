    ...
    <div class="kt-portlet kt-portlet--mobile">
        ...
        <div class="kt-portlet__body">
            ...
            <table class="table table-bordered" cellpadding="4">
                ...
                @canany(['Jadwal Kelas Publish'])
                @if ($jadwalKelas->is_publish == 0)
                <tr>
                    <td colspan="3" align="center" style="padding: 8px;">
                        <a href="{{ route('jadwal-kelas.publish.set-approve',['jadwal_kelas'=>$jadwalKelas,'status'=>'publish']) }}" class="btn btn-success btn-loading publishJadwal">Publish Kelas</a>
                    </td>
                </tr>
                @else
                <tr>
                    <td colspan="3" align="center" style="padding: 8px;">
                        <a href="{{ route('jadwal-kelas.publish.set-approve',['jadwal_kelas'=>$jadwalKelas,'status'=>'unpublish']) }}" class="btn btn-danger btn-loading unpublishJadwal">Batalkan Publish Kelas</a>
                    </td>
                </tr>
                @endif
                @endcanany
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('body').on('click','.publishJadwal', function (e) {
            e.preventDefault();
            var link = $(this).attr('href');
            swal.fire({
                title: 'Approve Jadwal?',
                text: 'Apakah Anda yakin untuk Publish Jadwal ini?',
                type: 'warning',
                allowOutsideClick: true,
                showConfirmButton: true,
                showCancelButton: true,
        ...
        $('body').on('click','.unpublishJadwal', function (e) {
            e.preventDefault();
            var link = $(this).attr('href');
            swal.fire({
                title: 'Batalkan Publish Jadwal?',
                text: 'Apakah Anda yakin untuk membatalkan Publish Jadwal ini?',
                type: 'warning',
                allowOutsideClick: true,
                showConfirmButton: true,
                showCancelButton: true,
